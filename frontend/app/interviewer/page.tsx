      ...
      <main className="flex-1 p-8 max-w-7xl mx-auto w-full">
        {activeTab === "dashboard" ? (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 backdrop-blur-sm">
                <span className="text-xs uppercase tracking-wider font-semibold text-slate-400">Total Evaluations</span>
                <div className="text-3xl font-extrabold text-slate-100 mt-2">{sessions.length}</div>
              </div>
              <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 backdrop-blur-sm">
              ...
              <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 backdrop-blur-sm">
                <span className="text-xs uppercase tracking-wider font-semibold text-amber-400">Avg Conceptual Accuracy</span>
              ...
              <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 backdrop-blur-sm">
                <span className="text-xs uppercase tracking-wider font-semibold text-cyan-400">RAG Knowledge Base</span>
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {filteredSessions.map((s) => (
                    <tr key={s.id} className="hover:bg-slate-800/30 transition">
                      <td className="p-4 font-semibold text-slate-200">{s.candidate_name || "Anonymous"}</td>
                      <td className="p-4 font-mono text-slate-400">{s.roll_number || "N/A"}</td>
                      <td className="p-4 capitalize text-slate-300">{(s.subject_key || "General").replace(/_/g, " ")}</td>
                      ...
                      <td className="p-4 text-amber-400 font-bold">{roundNum(s.avg_accuracy || 75)}%</td>
                      <td className="p-4 font-extrabold text-slate-100">{roundNum(s.avg_overall || 78)}/100</td>
                      <td className="p-4 text-right flex items-center justify-end gap-2">
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="p-1.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 rounded-lg border border-amber-500/30 transition"
            ...
            {selectedSessionId && (
              <motion.div
                ...
                className="bg-slate-900/90 border border-amber-500/30 rounded-2xl p-6 shadow-2xl space-y-6"
              >
                <div className="flex justify-between items-center border-b border-slate-800 pb-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1">Review student answers side-by-side with evaluator reference keys, RAG citations, and anti-cheat audit flags.</p>
                  </div>
                  <button
                    ...
                    <div key={idx} className="bg-slate-950 border border-slate-800 rounded-xl p-5 space-y-4">
                        ...
                        <div className="bg-emerald-950/20 border border-emerald-500/20 rounded-xl p-4 space-y-2">
                          ...
                          <div className="space-y-1.5">
                            {ans.citations.map((cit, cidx) => (
                              <div key={cidx} className="text-xs text-slate-300 bg-slate-950/60 p-2.5 rounded-lg border border-slate-800">
                                <div className="flex justify-between items-center text-[11px] font-semibold text-emerald-300 mb-1">
                                  ...
                                  <span className="text-slate-400 capitalize">{cit.verdict || "Verified"}</span>
                                </div>
                                <p className="italic text-slate-400">"{cit.quote_snippet || cit.topic}"</p>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      {ans.conceptual_feedback && (
                        <div className="text-xs bg-slate-900/60 p-3 rounded-lg border border-slate-800 text-slate-300">
                          <strong className="text-amber-400">Feedback: </strong> {ans.conceptual_feedback}
                        </div>
                      )}
          ...
          <div className="space-y-6">
            <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 space-y-6">
              <div className="flex justify-between items-start flex-wrap gap-4">
                <div>
                  ...
                  <h2 className="text-2xl font-bold text-slate-100 mt-1">Syllabus Ingestion & Document Vector Store</h2>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <select
                    ...
                    className="bg-slate-950 border border-slate-800 rounded-xl py-2 px-3 text-xs font-semibold text-slate-200 focus:outline-none focus:border-emerald-500"
                  >
              ...
              <div className="border-2 border-dashed border-slate-800 hover:border-emerald-500/50 rounded-2xl p-8 text-center transition bg-slate-950/40">
                <input
                  type="file"
                  ref={fileInputRef}
                  accept=".pdf,.txt,.md"
                ...
                <div className="max-w-md mx-auto space-y-3">
                  ...
                  <h3 className="font-bold text-sm text-slate-200">Upload Course Material or Syllabus</h3>
                  ...
                  <button
                    ...
                    className="px-6 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-slate-950 font-bold rounded-xl text-xs transition shadow-lg inline-flex items-center gap-2"
                  >
                    {isUploadingRag ? (
                      <>
                ...
                {ragSources.map((source, idx) => (
                  <div key={idx} className="bg-slate-950 border border-slate-800 rounded-xl p-4 space-y-3">
                    ...
                    <div className="text-2xl font-extrabold text-slate-100">
                      {source.total_chunks} <span className="text-xs text-slate-400 font-normal">Indexed Chunks</span>
                    </div>
                    <div className="space-y-1 pt-1">
                      {source.documents.map((doc: any, dIdx: number) => (
                        <div key={dIdx} className="text-xs text-slate-300 flex justify-between items-center bg-slate-900/80 px-2.5 py-1.5 rounded-lg border border-slate-800">
            ...
            <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 space-y-6">
              <div>
                <span className="text-xs uppercase tracking-wider font-semibold text-amber-400">Evaluation Sandbox</span>
                <h3 className="text-lg font-bold text-slate-100 mt-1">Test Similarity Search & RAGAS Grounding Live</h3>
                <p className="text-xs text-slate-400 mt-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      className="px-5 py-2 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-slate-950 font-bold rounded-xl text-xs transition shadow-md flex items-center gap-1.5"
                    >
                ...
                <div className="bg-slate-950 rounded-xl border border-slate-800 p-4 space-y-4 max-h-[450px] overflow-y-auto">
                  {ragEvalResult ? (
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div className="bg-slate-900 p-2.5 rounded-lg border border-slate-800">
                          <span className="text-slate-400 text-[10px] uppercase block">Faithfulness</span>
                          <span className="text-base font-bold text-emerald-400">{ragEvalResult.faithfulness_score}%</span>
                        </div>
                        <div className="bg-slate-900 p-2.5 rounded-lg border border-slate-800">
                          <span className="text-slate-400 text-[10px] uppercase block">Relevance</span>
                          <span className="text-base font-bold text-amber-400">{ragEvalResult.answer_relevance}%</span>
                        </div>
                      </div>
                        ...
                        <div className="space-y-1.5 text-xs">
                          ...
                          {ragEvalResult.citations.map((c: any, i: number) => (
                            <div key={i} className="p-2 bg-slate-900 rounded-lg border border-slate-800 text-[11px]">
                              ...
                              <span className="italic text-slate-300">"{c.quote_snippet}"</span>
                            </div>
                          ))}
                        </div>
                      )}
                      ...
                      {ragEvalResult.conceptual_feedback && (
                        <div className="text-xs bg-slate-900 p-3 rounded-lg text-slate-300 border border-slate-800">
                          <strong className="text-amber-400">Feedback: </strong> {ragEvalResult.conceptual_feedback}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      {ragTestResults.map((res, i) => (
                        <div key={i} className="p-3 bg-slate-900/90 rounded-lg border border-slate-800 text-xs space-y-1">
                          ...
                          <p className="text-slate-300 text-[11px] leading-relaxed line-clamp-4">{res.text}</p>
                        </div>
                      ))}
                    </div>
                  ) : (
